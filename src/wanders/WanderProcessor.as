	...
	public class WanderProcessor
	{
		...
		public function resetRandomPoint(x : int, y : int) : void
		{
			...
			for (i = 1; i < length; i++)
			{
				...
				tempPoint.x = x + MapMath.randomPlusMinus(App.mapWidth / 3);
				tempPoint.y = y + MapMath.randomPlusMinus(App.mapHeight / 3);
				if (tempPoint.x < 0) tempPoint.x = 0;
				else if (tempPoint.x > App.mapWidth ) tempPoint.x = App.mapWidth ;
				if (tempPoint.y < 0) tempPoint.y = 0;
				else if (tempPoint.y > App.mapHeight ) tempPoint.y = App.mapHeight ;
			}
			pointList.push(tempPoint);
		}
